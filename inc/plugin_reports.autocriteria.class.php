...
class GenericDropdownCriteria extends AutoCriteria {
	...
	public function getSubName() {
		return " " . $this->getCriteriaLabel() . " : " . getDropdownName($this->getTable(), $this->getParameterValue());
	}
...
class LocationCriteria extends GenericDropdownCriteria {
	function __construct($report) {
		global $LANG;
		parent :: __construct($report, "location", "glpi_dropdown_locations", $LANG["common"][15]);
	}
...
class DateIntervalCriteria extends AutoCriteria {
	...
	public function getSqlCriteriasRestriction($link = 'AND') {
		return $link . " " .
		$this->getSqlField() . " >= '" . $this->getParameter("startdate") . " 00:00:00' AND " . $this->getSqlField() . "<='" . $this->getParameter("enddate") . " 23:59:59' ";
	}

	function getSubName() {
		global $LANG;
		return (isset ($LANG['plugin_reports']['subname'][$this->getName()]) ? $LANG['plugin_reports']['subname'][$this->getName()] : '') .
		" (" . convDate($this->getParameter("startdate")) . "," . convDate($this->getParameter("enddate")) . ")";
	}
}
...
class TimeIntervalCriteria extends AutoCriteria {
	...
	function getSqlCriteriasRestriction($link = 'AND') {
		if ($this->getParameter("starttime") < $this->getParameter("endtime")) {
			...
			return " $link TIME(" . $this->getSqlField() . ") >= '" . $this->getParameter("starttime") .
			":00' AND TIME(" . $this->getSqlField() . ") < '" . $this->getParameter("endtime") . ":00' ";
		} else {
			// ex time < 08:00:00 or 18:00:00 <= time
			return " $link (TIME(" . $this->getSqlField() . ") >= '" . $this->getParameter("starttime") .
			":00' OR TIME(" . $this->getSqlField() . ") < '" . $this->getParameter("endtime") . ":00') ";
		}
	}
	...
	function getSubName() {
		global $LANG;
		return " " .
		 (isset ($LANG['plugin_reports']['subname'][$this->getName()]) ? $LANG['plugin_reports']['subname'][$this->getName()] : '') .
...
class TicketStatusCriteria extends GenericDropdownCriteria {
	function __construct($report) {
		global $LANG;
		parent :: __construct($report, "status", "glpi_dropdown_status", $LANG['joblist'][0]);
	}
...
class StatusCriteria extends GenericDropdownCriteria {
	function __construct($report) {
		global $LANG;
		parent :: __construct($report, "state", "glpi_dropdown_state", $LANG['joblist'][0]);
	}
}
...
class TicketCategoryCriteria extends GenericDropdownCriteria {
	function __construct($report) {
		global $LANG;
		parent :: __construct($report, "category", "glpi_dropdown_tracking_category", $LANG['common'][36]);
	}
}
...
class UserCriteria extends GenericDropdownCriteria {
	...
	public function displayDropdownCriteria() {
		dropdownUsers($this->getName(),$this->getParameterValue(),'all',0,$this->getDisplayComments(),$this->getEntityRestrict());
	}
}
...
class UserTypeCriteria extends GenericDropdownCriteria {
	function __construct($report) {
		global $LANG;
		parent :: __construct($report, "type", "glpi_dropdown_user_types", $LANG['common'][34]." : ".$LANG['common'][17]);
	}
}
...
class UserTitleCriteria extends GenericDropdownCriteria {
	function __construct($report) {
		global $LANG;
		parent :: __construct($report, "title", "glpi_dropdown_user_titles", $LANG['common'][34]." : ".$LANG['common'][57]);
	}
}
